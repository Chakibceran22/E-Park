          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-2 sm:space-y-0 text-gray-600 mb-4">
            ...
            <div className="flex items-center">
              ...
              <span className="text-sm">
                {parkingLot.pricePerHalfHour} DZD/30min
              </span>
            </div>
          </div>
                  ...
                  <div className="mt-6 sm:mt-8 mb-6 sm:mb-8 min-w-max">
                    {["A", "B", "C", "D", "E", "F"].map((row) => (
                      <div key={row} className="flex items-center mb-2 sm:mb-4">
                        <div className="w-6 sm:w-8 text-center font-bold text-sm sm:text-lg mr-2 sm:mr-4">
                          {row}
                        </div>
                        <div className="flex space-x-0.5 sm:space-x-1">
                          {Array.from({ length: 20 }, (_, spotIndex) => {
                            const spaceId = `${row}${(spotIndex + 1)
                              .toString()
                              .padStart(2, "0")}`;
                            ...
                            return (
                              <button
                                ...
                                disabled={
                                  !space?.isAvailable ||
                                  reservedSpots.has(spaceId)
                                }
                                className={`w-6 h-8 sm:w-8 sm:h-12 border-2 rounded text-xs font-medium transition-all duration-300 ${getSpaceColor(
                                  space
                                )} ${
                                  space?.isAvailable &&
                                  !reservedSpots.has(spaceId)
                ...
                {selectedSpot ? (
                      ...
                      >
                        {isProcessing ? (
                          ...
                          <>
                            ...
                            Reserve - {calculateTotalCost()} DZD
                          </>
                        )}
                      </Button>
                    </div>
                  ...
                  <div className="text-center p-6 sm:p-8 text-gray-500">
                    ...
                    <p className="text-sm sm:text-base">
                      Select an available parking space to continue
                    </p>
                  </div>
                )}
              </CardContent>
